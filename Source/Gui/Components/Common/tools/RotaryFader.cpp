...
void RotaryFader::paint(juce::Graphics& g) {
    ...
    auto bounds = getLocalBounds().toFloat().withTrimmedTop(16);
    if (label.has_value()) {
        bounds = bounds.withTrimmedBottom(16);
    }
    ...
    if (label.has_value()) {
        auto labelBounds = getLocalBounds().removeFromBottom(14);
        g.setColour(juce::Colour::fromString("#FF2F2C3F"));
...
float RotaryFader::getVolumeDb() const {
    ...
    if (clampedValue >= kZeroPosition) {
        const float t = (clampedValue - kZeroPosition) / (1.0f - kZeroPosition);
        return juce::jmap(t, 0.0f, 1.0f, 0.0f, kMaxVolumeDb);
    }
    const float t = clampedValue / kZeroPosition;
...
void RotaryFader::setLabel(std::optional<juce::String> newLabel) {
